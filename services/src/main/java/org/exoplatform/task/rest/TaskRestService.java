...
public class TaskRestService implements ResourceContainer {
  ...
  public Response getTasks(@Parameter(description = "Type of task to get (all, incoming, overdue)") @QueryParam("status") String status,
                           ...
                           @Parameter(description = "Field to sort by", required = false) @QueryParam("sort") String sortField,
                           @Parameter(description = "Sort order (asc or desc)") @QueryParam("order") String sortDirection,
    ...
    try {
      ...
      if (StringUtils.isBlank(query)) {
        ...
        tasks = taskService.findTasks(new TaskFilter(currentUser, query, memberships, sortField, sortDirection, offset, limit));
